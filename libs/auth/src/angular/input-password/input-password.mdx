...
- `userId` - allows the `InputPasswordComponent` to do things like get the user's `kdfConfig` and
  `salt`, and perform validation prior to user key rotation (if selected) on the parent
...
- An existing user in a TDE org logs in after an org admin upgraded the user to have the reset
  password permission ("manage account recovery"), which now requires the user to have/set their
  initial password
...
Click through the individual Stories in Storybook to see how the `InputPasswordFlow` determines
